...
class IBookingUseCase(ABC):
    ...
    @abstractmethod
    async def get_calendar_schedule_booking(
        self,
        session: AsyncSession,
        service_id: int,
        company_id: int,
...
class BookingUseCase(IBookingUseCase):
    ...
    async def save_booking(
        ...
        return await self.booking_repository.save_booking(
            session, service_id, client_id, time_start, time_end
        )
    ...
    async def is_booking_time_in_work_schedule(
        ...
        work_schedule = await self.company_repository.get_work_schedule_by_company_id(
            session, company_id
        )
    ...
    @staticmethod
    def get_intervals(
        start_time: time, end_time: time, service_duration_min: int
    ...
    def remove_overlapping_intervals(data):
        ...
        for key, intervals in data.items():
            ...
            unique_intervals = [
                interval for interval in intervals if count[interval] == 1
            ]
    ...
    async def get_services_intervals(
        self, services: list[ServiceEntity], work_schedule: list
        ...
        for ws in work_schedule:
            ...
            for service in services:
                list_intervals = self.get_intervals(
                    work_start, work_end, service.duration
                )
    ...
    async def filter_intervals_by_service_duration(
        intervals: dict[int, list[tuple[time, time]]], service_duration_min: int
    ) -> dict[int, list[tuple[time, time]]]:
        if service_duration_min <= 0:
            raise ValueError("Длительность услуги должна быть положительной")
        ...
        for key in intervals:
            ...
            for start, end in intervals[key]:
                ...
                if end_dt < start_dt:
                    raise ValueError(
                        f"Некорректный интервал: {start}–{end} (конец раньше начала)"
                    )
    ...
    async def get_calendar_schedule_booking(
        ...
        services = await self.service_repository.get_services_by_company_id(
            session, company_id
        )

        services_intervals = await self.get_services_intervals(services, work_schedule)

        bookings_by_service_id = (
            await self.booking_repository.get_booking_by_service_id(session, service_id)
        )
        ...
        for booking in bookings_by_service_id:
            ...
            t = (
                time(
                    hour=booking.time_start.time().hour,
                    minute=booking.time_start.time().minute,
                ),
                time(
                    hour=booking.time_end.time().hour,
                    minute=booking.time_end.time().minute,
                ),
            )
        ...
        non_overlapping_intervals = self.remove_overlapping_intervals(
            services_intervals
        )
        ...
        for day in range(self.calendar_schedule_settings.calendar_schedule_limit_days):
            ...
            if now.isoweekday() in days_of_week_to_work:
                if now.day in days_already_booked:
                    time_to_book = (
                        await self.filter_intervals_by_service_duration(
                            non_overlapping_intervals, service.duration
                        ),
                    )
                    time_to_book = (
                        time_to_book[0][now.isoweekday()]
                        if isinstance(time_to_book, tuple)
                        else time_to_book[now.isoweekday()]
                    )
                    ...
                    for ws in work_schedule:
                        if ws["day_of_week"] == now.isoweekday():
                            ...
                            for t in time_to_book:
                                ...
                                if (
                                    time_to_book_start < work_start
                                    or time_to_book_end > work_end
                                ):
                                    continue
                                else:
                    ...
                    if intervals is not None:
                        for interval in non_overlapping_intervals.get(now.isoweekday()):
                            for ws in work_schedule:
                                ...
                                work_start = time(
                                    int(work_start.split(":")[0]),
                                    int(work_start.split(":")[1]),
                                )
                                work_end = time(
                                    int(work_end.split(":")[0]),
                                    int(work_end.split(":")[1]),
                                )

                                work_start_dt = datetime.combine(
                                    datetime.today().date(), work_start
                                )
                                work_end_dt = datetime.combine(
                                    datetime.today().date(), work_end
                                )
                                ...
                                start = int(
                                    datetime.combine(
                                        datetime.today().date(),
                                        time(interval[0].hour, interval[0].minute),
                                    ).timestamp()
                                )
                                end = int(
                                    datetime.combine(
                                        datetime.today().date(),
                                        time(interval[1].hour, interval[1].minute),
                                    ).timestamp()
                                )
        ...
        return {**service.to_dict(), "schedule": schedule}
    ...
    async def get_booking_by_service_id_and_client_id(
        ...
        return await self.booking_repository.get_booking_by_service_id_and_client_id(
            session, service_id, client_id
        )
    ...
    async def update_booking_by_id(
        ...
        return await self.booking_repository.update_booking_by_id(
            session, booking_id, data_to_update
        )
